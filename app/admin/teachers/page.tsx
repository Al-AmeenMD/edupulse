...
export default function TeachersPage() {
  ...
  const [resetTargetTeacher, setResetTargetTeacher] = useState<Teacher | null>(null);
  ...
  function handleOpenResetPasswordModal(teacher: Teacher) {
    setResetTargetTeacher(teacher);
  ...
  async function handleConfirmResetPassword(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/users/${resetTargetTeacher.user.id}/reset-password`, {
        method: "POST",
        headers: {
  ...
  return (
    <div className="space-y-8">
      ...
      {isResetPasswordModalOpen && resetTargetTeacher && (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-5 border border-slate-100">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Reset password for {resetTargetTeacher.user.firstName} {resetTargetTeacher.user.lastName} ({resetTargetTeacher.user.email})
                </p>
              </div>
              <button
