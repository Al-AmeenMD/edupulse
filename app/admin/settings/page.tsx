...
import AcademicSessionsManager from "@/components/admin/AcademicSessionsManager";
...
export default function AdminSettingsPage() {
  const [activeTab, setActiveTab] = useState<"sessions" | "id_template" | "security">("sessions");
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Manage academic calendar sessions, student ID templates, and security for {schoolName || "your school"}.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
        ...
        <button
          type="button"
          onClick={() => setActiveTab("id_template")}
          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
            activeTab === "id_template"
              ? "bg-blue-600 text-white shadow-2xs"
              : "bg-slate-100 text-slate-600 hover:bg-slate-200"
          }`}
        >
      ...
      {/* Tab 1: Academic Sessions & Terms */}
      {activeTab === "sessions" && <AcademicSessionsManager />}

      {/* Tab 2: Student ID Template */}
      {activeTab === "id_template" && (
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200/80 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <span className="text-slate-600">Full 4-digit current year (e.g. {new Date().getFullYear()})</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <code className="px-2 py-0.5 bg-white border border-slate-200 rounded-md font-mono font-bold text-blue-600 shrink-0">
                      {"{LEVEL}"}
                    </code>
                    <span className="text-slate-600">Student admission level (e.g. JSS1, SS2)</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <code className="px-2 py-0.5 bg-white border border-slate-200 rounded-md font-mono font-bold text-blue-600 shrink-0">
                      {"{SEQ:N}"}
                    </code>
                    <span className="text-slate-600">Auto-incrementing sequence padded to N digits (e.g. {"{SEQ:3}"} $\rightarrow$ 001)</span>
                  </div>
                </div>
              </div>
      ...
      {/* Tab 3: Security & Password Card */}
      {activeTab === "security" && (
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
          <div className="p-6 border-b border-slate-100">
            <h2 className="text-lg font-bold text-slate-900">Security & Account Password</h2>
            <p className="text-xs text-slate-500 mt-1">
              Update your account password to maintain security.
            </p>
          </div>

        <form onSubmit={handleChangePassword} autoComplete="off" className="p-6 space-y-4 max-w-xl">
          {passwordSuccess && (
